...
- `posts_clauses` priority 99 (`order_category_archive_in_stock_first`): **on `product_cat` archives only** (frontend main query), lists **in-stock products before On Order (backorder)** ones — prepends a `CASE WHEN stock_status='instock' THEN 0 ELSE 1 END` primary sort with the shopper's chosen WC "Sort by" as the secondary — and **restricts the visible pool** to `stock_status IN ('instock','onbackorder')` AND `_manage_stock='yes'` (out-of-stock / non-managed products vanish). Joins its own alias (`trm_sml`) of `wc_product_meta_lookup` + `trm_ms` postmeta so it never collides with WC's price/popularity/rating join. Runs after WC's ordering clauses (prio 10) and **on top of** the priority-99 `pre_get_posts` gates (no-`_price` + Coming Soon), which still apply.
...
- **On-site registration (item 29):** opt-in per event via the `event_enable_website_registration` toggle. When on, the external link is ignored and shoppers book seats through a modal on the event page (capped by `event_participants`; "Event is Fully Booked" when none remain), and the client manages the roster from a meta box on the event editor. Backed by a new **`{prefix}trem_event_registrations`** table where **seats-taken is always a live `COUNT(*)`** (no stored counter, so deleting a row frees a seat). AJAX: `trem_register_for_event` (+nopriv, front-end submit with duplicate-email guard + last-seat re-count + confirmation/admin emails) and `trem_admin_update_registration` / `trem_admin_delete_registration` (per-event `edit_post` cap). DB schema versioned via the new `trem_db_version` option (`TREM_Event_Registrations`; `TREM_Registration_Ajax`; `TREM_Registrations_Admin`). Theme assets: `assets/js/trm-event-registration.js` + `assets/scss/event-registration.scss`.
