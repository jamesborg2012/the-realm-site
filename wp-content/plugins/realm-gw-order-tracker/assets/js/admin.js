            ...
            .done(res => {
                ...
                const rows = items
                    .map(i => {
                        ...
                        const rowClass = i.is_online_only ? 'gwot-online-only' : '';
                        const badge = i.is_online_only ? '<span class="gwot-badge">Online Only</span>' : '';

                        return `
        <tr class="${rowClass}" data-item-id="${i.item_id}">
      <td>${i.title} ${badge}</td>
      <td>${sku}</td>
      <td>${i.qty}</td>
